...
###############################################################
## Function to obtain performance of estimators in Lad Reg
...
## ncores_par = number of cores for parallelization
...
lad_reps <- function(max_sam = 1e5, Rep = 1, nparm = 5, A = diag(nparm), cns = c(0.1, 1), ncores_par = 1, 
                            eta_cns = 0.5, sam_siz = c(5e4, 1e5, 2e5, 5e5, 8e5, 1e6, 5e6, 1e7), qlev = 0.95, alp = .51, 
                            burn_in = 1000, nam_matrix = "indep", cns1 = 1, tau = 0.5)
{
  ...
  n <- sam_siz[length(sam_siz)]          # Largest sample size
  ...
  # For oracle
  volm_orc             <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  marg_volm_orc        <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  cover_orc            <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  marg_sim_cov_orc     <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))

  # For IBS estimator
  volm_ibs             <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  marg_volm_ibs        <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  cover_ibs            <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  marg_sim_cov_ibs     <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  forb_ibs             <- matrix(rep(0, length(sam_siz)*Rep), nrow = Rep, ncol = length(sam_siz), dimnames = list( 1 : Rep, sam_siz))
  
  # For EBS estimator
  volm_ebs             <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  marg_volm_ebs        <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))  
  cover_ebs            <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln)) 
  marg_sim_cov_ebs     <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  forb_ebs             <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  ...
  marg_volm_ebs_ls     <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  cover_ebs_ls         <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  marg_sim_cov_ebs_ls  <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
  forb_ebs_ls          <- array(rep(0, cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln))
 
  # For relative volume comparisons
  ratio_ibs_ebs        <- array(rep(0, nparm * cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln, nparm), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln, 1 : nparm))
  ratio_ibs_ebs_ls     <- array(rep(0, nparm * cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln, nparm), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln, 1 : nparm))
  ratio_ebs_ls_ebs     <- array(rep(0, nparm * cns_ln * Rep * length(sam_siz)), dim = c(length(sam_siz), Rep, cns_ln, nparm), dimnames = list(sam_siz, 1 : Rep, 1 : cns_ln, 1 : nparm))

  
  registerDoParallel(cores = min(Rep, ncores_par))  
  
  final_values <- foreach(cn = 1:Rep, .combine = "comb", .packages = c("MASS","mcmcse", "mvtnorm", "smoothmest"), .export = c( 'ebs_batch_mean', 'ibs_jasa_mean', 'sqrt_mat', 'grad_lad', 'new.sim.int', 'opt_beta_fn', 'b_our'), .multicombine=TRUE,
                            .init = list(list(), list(), list(),list(), list(), list(), list(),list(), list(),list(), list(),list(), list(),list(), list(),list(), list(),list(), list(), list(), list(), list() )) %dopar% 
  {
    ...
    { 
      sg_ct <- sg_ct_full[1 : sam_siz[smpl], ]  # main process inside this loop
      ...
      { 
        ...
        { 
          ...
          marg_volm_ebs_ls[smpl, cn, count] <- (prod(leng_ebs_ls) / volm_ebs_ls[smpl, cn, count])^(1 / nparm)          
  ...
  fil_nam <- paste("out/lad_", nam_matrix, "_n_", n, "_dim_", nparm, ".RData", sep = "")
  save(marg_sim_cov_orc, marg_sim_cov_ibs, marg_sim_cov_ebs, marg_sim_cov_ebs_ls, volm_orc, marg_volm_orc, marg_volm_ibs, marg_volm_ebs, marg_volm_ebs_ls, ratio_ibs_ebs, ratio_ibs_ebs_ls, ratio_ebs_ls_ebs, cover_orc, cover_ibs, cover_ebs, cover_ebs_ls, volm_ibs, volm_ebs, volm_ebs_ls, forb_ibs, forb_ebs, forb_ebs_ls, file = fil_nam)
}
