...
ebs_bias <- function(n, al, lug = 0)
{
  ...
  ## packaages for parallel computation
  require(foreach)
  require(doParallel)
  ...
  aa <- foreach(j = 1:(an-1), .combine=cbind)%dopar% {
    ...
    for (k in (j+1):an) 
    {
      ...
      {
        abc = abc + q^(-al)*sum( (1-q^(-al))^( q - c(((j-1)*bn+1):(j*bn)) ) )
      }
    }
    abc
  }
...
{ 
  print(sam_siz[i])             ## running sample size
  
  res1 = ebs_bias(sam_siz[i], al = al, lug = 0) ## EBS bias
  res2 = ebs_bias(sam_siz[i], al = al, lug = 1) ## EBS bias with double sample size
  
  out_ebs[i,(2*j-1):(2*j)] = c(res1, 2*res2 - res1) ## c(EBS bias, Lugsail-EBS bias)
}
