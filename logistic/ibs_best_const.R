...
nparm = 5
A = diag(nparm)
cns1 <- seq(0.005,0.1, by = 0.005) #First set length(seq(0.1,4, by = 0.2)) for dim 5
eta_cns <- 0.5
...
cover_ibs <- forb_ibs <- volm_ibs <- matrix(rep(0,length(sam_siz)*length(cns1)),nrow = length(cns1), ncol = length(sam_siz), dimnames = list( 1 : length(cns1), sam_siz))
...
final_values <- foreach(cn = 1 : Rep, .combine = "comb", .packages = c("MASS","mcmcse"), .export = c( 'sqrt_mat'), .multicombine=TRUE,
                        .init=list(list(), list(), list() )) %dopar% {
                            ...
                            for ( smpl in 1 : length(sam_siz)) 
                            {
                              ...
                              asg <- colMeans(sg_ct)
                              for (k  in 1 : length(cns1)){
                                cns <- cns1[k]
                              ibs_mean     <- ibs_jasa_mean(sg_ct, alp, cns = cns )
                              
                              forb_ibs[k, smpl]  <-  norm(ibs_mean - sigm, "F")/ norm(sigm, "F") 
                              volm_ibs[k, smpl]  <- (det(ibs_mean)) ^ (1 / nparm)
                              cover_ibs[k, smpl] <- as.numeric(sam_siz[smpl]  * t(asg - parm) %*% qr.solve(ibs_mean) %*% (asg - parm) <= crt_val)
                          ...
                          list(cover_ibs, forb_ibs, volm_ibs)    
                        }
...
for( k in 1 : Rep){
  ...
  cover_ibs <- cover_ibs + final_values[[1]][[k]]
  forb_ibs <- cover_ibs + final_values[[2]][[k]]
  volm_ibs <- cover_ibs + final_values[[3]][[k]]
}




fil_nam <- paste("out/log_ibs_select_dim_",nparm,"_const.RData",sep="")
save(cover_ibs, forb_ibs, volm_ibs, file = fil_nam)
