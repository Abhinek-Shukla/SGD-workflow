...
log_batch_fn <- function(max_sam = 1e5, Rep = 1, nparm = 5, A = diag(nparm), cns = c(0.1, 1), ncores_par = 1, eta_cns = 0.5, eta_typ = 1,  sam_siz = c(1e5,2e5,5e5,8e5,1e6,5e6,1e7), qlev = 0.95, alp = .51, burn_in = 1000, nam_matrix = "indep"){
...
sigm_inv <- get(paste("sigm_inv_",nam_matrix,"_nparm_", nparm, sep= ""))
  ...
  fil_nam <- paste("out/logistic_", nam_matrix, "_n_",max_sam,"_dim_",nparm,"_eta_", eta_typ , ".RData",sep="")
