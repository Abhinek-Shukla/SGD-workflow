...
linear_reps <- function(max_sam = 1e5, Rep = 1, nparm = 5, 
                        A = diag(nparm), cns = c(0.1, 1), 
                        ncores_par = 1, eta_cns = 0.5, 
                        sam_siz = c(5e4, 1e5, 2e5, 5e5, 8e5, 
                                    1e6, 5e6, 1e7), qlev = 0.95, 
                        alp = .51, burn_in = 1000, 
                        nam_matrix = "indep", cns1 = 1)
{
  ...
  final_values <- foreach(cn = 1:Rep, .combine = "comb", .packages = c("MASS","mcmcse"
                          , "mvtnorm"), .export = c( 'ebs_batch_mean', 'ibs_jasa_mean', 
                          'sqrt_mat', 'grad_lin', 'new.sim.int', 'opt_beta_fn', 'b_our'), 
  ...
  save(marg_sim_cov_orc, marg_sim_cov_ibs, marg_sim_cov_ebs, marg_sim_cov_ebs_ls,
       ...
       cover_ebs, cover_ebs_ls, volm_ibs, volm_ebs, volm_ebs_ls, forb_ibs, forb_ebs,
